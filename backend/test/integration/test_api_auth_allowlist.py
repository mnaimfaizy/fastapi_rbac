...
from app.utils.token import get_valid_tokens, revoke_user_tokens, token_is_allowlisted
...
async def test_oauth2_first_login_writes_allowlist_and_logout_rejects(
    ...
    members = await get_valid_tokens(server_redis, user_id, TokenType.ACCESS)
...
async def test_json_login_writes_access_and_refresh_allowlist(
    ...
    access_members = await get_valid_tokens(server_redis, user_id, TokenType.ACCESS)
    refresh_members = await get_valid_tokens(server_redis, user_id, TokenType.REFRESH)
...
async def test_refresh_rejected_when_allowlist_empty(
    ...
    await revoke_user_tokens(server_redis, user_id, TokenType.REFRESH)
