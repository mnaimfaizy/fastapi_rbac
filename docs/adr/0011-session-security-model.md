...
- Decision 8 changes which address the rate limiter keys on, which silently re-keys HTTP rate limiting from one global bucket to genuine per-client buckets. Issue [#100](https://github.com/mnaimfaizy/fastapi_rbac/issues/100) assumes an IP fallback that does not currently work; it depends on this decision landing first.
