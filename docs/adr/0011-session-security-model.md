...
- Decision 8 changes which address the rate limiter keys on, which silently re-keys HTTP rate limiting from one global bucket to genuine per-client buckets. Issue [#100](https://github.com/mnaimfaizy/fastapi_rbac/issues/100) needed that fallback before it could key authenticated traffic by user; it now keys `user:{id}` when identity is established and `ip:{address}` otherwise ([ADR 0008](./0008-slowapi-sole-http-rate-limit.md)).
- Decision 4 was carried out in [#204](https://github.com/mnaimfaizy/fastapi_rbac/issues/204): the four unimplemented session settings no longer exist in configuration, environment examples, or security docs that had advertised them as live controls. A leftover `.env` that still lists the old keys is ignored (`extra="ignore"`), so a stale operator file does not break a deploy.
...
- A client that assumed `POST /logout` signed the account out everywhere will keep other sessions alive after a single-device logout. `POST /logout/all` is the named replacement. The first-party SPA keeps ordinary Logout on `POST /logout` and posts `POST /logout/all` from a separate **Log out everywhere** control (#252).
